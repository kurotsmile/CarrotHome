...
function oauth_callback_json(array $payload, int $statusCode = 200): void
{
...
function oauth_callback_redirect_error(string $message): void
{
...
function oauth_callback_login_user(PDO $pdo, string $email, string $name, string $provider, string $avatar = ''): void
{
    $email = trim($email);
...
if (!$pdo instanceof PDO) {
    oauth_callback_redirect_error($db_error ?? 'Không thể kết nối database.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'supabase_token') {
    try {
        ...
        $user = oauth_callback_request($projectUrl . '/auth/v1/user', [
            'headers' => [
                'apikey: ' . $apiKey,
...
if (isset($_GET['error'])) {
    oauth_callback_redirect_error((string)($_GET['error_description'] ?? $_GET['error']));
}
...
(function () {
  ...
  if (!token) {
    window.location.href = 'login.php?mode=register&oauth_error=' + encodeURIComponent('Không nhận được Supabase access token.');
    return;
  }
  fetch('oauth-callback.php', {
    ...
    .catch(function (error) {
      window.location.href = 'login.php?mode=register&oauth_error=' + encodeURIComponent(error.message || 'OAuth failed');
    });
})();
</script>
</body>
...
try {
    ...
    if ($provider === 'github') {
        ...
        $profile = oauth_callback_request('https://api.github.com/user', [
            'headers' => ['Authorization: Bearer ' . $accessToken, 'User-Agent: CarrotHome', 'Accept: application/vnd.github+json'],
        ]);
        $emails = oauth_callback_request('https://api.github.com/user/emails', [
            'headers' => ['Authorization: Bearer ' . $accessToken, 'User-Agent: CarrotHome', 'Accept: application/vnd.github+json'],
        ]);
        ...
        oauth_callback_login_user($pdo, $email, (string)($profile['name'] ?? $profile['login'] ?? ''), 'github', (string)($profile['avatar_url'] ?? ''));
    } elseif ($provider === 'google') {
        $token = oauth_callback_request('https://oauth2.googleapis.com/token', [
        ...
        $accessToken = (string)($token['access_token'] ?? '');
        $profile = oauth_callback_request('https://openidconnect.googleapis.com/v1/userinfo', [
            'headers' => ['Authorization: Bearer ' . $accessToken],
        ]);
        oauth_callback_login_user($pdo, (string)($profile['email'] ?? ''), (string)($profile['name'] ?? ''), 'google', (string)($profile['picture'] ?? ''));
    } else {
